  ...
  <div class="col-xs-12 col-md-2 user-sidebar nopadding">
    ...
    <div v-if="!!info && accessFound">
      <div class="col-md-12 sidebar-header">
        <img src="/img/user_avatar.png" alt="" class="img img-circle img-responsive profile-img">
...
export default {
  ...
  methods: {
    onMenuClick(tag) {
      if (tag === 'rms_sites') {
        window.location.href = `/rms-site?user_id=${this.info.id}`
    ...
    onProfileEdited(profile) {
      if (profile) {
        this.info = profile;
        EventBus.$emit('menu-clicked', this.current, this.info, this.access[this.current]['meta']);
      }
    },
...
.manage-menu > li:hover, .manage-menu > li.active {
    background: rgb(255, 255, 255);
    border-left: 2px solid rgb(84, 72, 236);
}
.manage-menu > li:hover > span, .manage-menu > li.active > span {
    color: rgb(84, 72, 236);
}
</style>
